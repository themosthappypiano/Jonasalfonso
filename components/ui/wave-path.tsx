...
export function WavePath({ className, ...props }: WWavePathProps) {
  ...
  useEffect(() => {
    ...
    if (path.current) {
      path.current.setAttributeNS(
        null,
        "d",
        `M0 100 Q${width * 0.2} 100, ${width} 100`,
      );
    }
  }, []);
  ...
  return (
    <div className={cn("relative h-px w-[70vw]", className)} {...props}>
      <button
        type="button"
        aria-label="Wave interaction area"
        onMouseEnter={manageMouseEnter}
        onMouseMove={manageMouseMove}
        onMouseLeave={manageMouseLeave}
        className="relative -top-5 z-10 h-10 w-full border-0 bg-transparent p-0 hover:-top-[150px] hover:h-[300px]"
      />
      <svg
        aria-hidden="true"
